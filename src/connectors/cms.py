...
MONTH_PATTERN = re.compile(
    r"\b(december|november|october|september|august|july|june|may|april|march|february|january)\b",
    re.IGNORECASE,
)
...
def maybe_get_updated_files(files: list[Path], use_update: bool) -> list[Path]:
    ...
    #         with Console(file=sys.stderr) as console:
...
def download_cms_icd_version(download_path: Path, year: int, use_update: bool) -> None:
    ...
    if not version_links:
        ...
        raise ValueError(
            f"No files found for year {year}. Available years: {available_years}"
        )
    ...
    new_download_links = [
        ...
        if Path(link.name).name not in downloaded_file_names
    ]
    ...
    for file_path in new_download_links:
        ...
        with Console(file=sys.stderr) as console:
            console.print(
                f"[bold blue]Downloading ICD version {Path(file_url).name}...[/bold blue]"
            )
    ...
    with Console(file=sys.stderr) as console:
        console.print(
            f"[bold green]Downloaded ICD version {year} successfully![/bold green]"
        )
