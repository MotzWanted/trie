...
try:
    import pymupdf as fitz  # Prefer the official PyMuPDF import name
except Exception:  # pragma: no cover - fallback if only legacy alias is available
    import fitz  # type: ignore[no-redef]


from src import models, xml_utils
from src.base import TRIE_CACHE_DIR, Trie
from src.connectors.cms import download_cms_icd_version
...
class ICD10Trie(Trie):
    ...
    def parse_guidelines(self) -> None:
        ...
        for level, title, page in track(
            ...
            description="Parsing Guidelines",
    ...
    def insert_cm_chapters_into_trie(self, chapters: list[models.CmChapter]) -> None:
        ...
        for ch in track(
            chapters,
            description="Parsing ICD-10-CM chapters",
            console=Console(file=sys.stderr),
        ):
